...
{-
    This is originally written by AndrasKovacs. Which do parsing like a CString.
    https://github.com/AndrasKovacs/setoidtt/blob/master/setoidtt/flatparse/FlatParse.hs
-}
...
-- | A specialised satisfy function that accepts 4 predicates
...
satisfy' f1 f2 f3 f4 = Parser \r eob buf n -> case eqAddr# eob buf of
  ...
  _ -> case derefChar8# buf of
    c1 -> case c1 `leChar#` '\x7F'# of
      1#
        | f1 (C# c1) -> OK# (C# c1) (plusAddr# buf 1#) n
        | otherwise -> Fail#
      _ -> case eqAddr# eob (plusAddr# buf 1#) of
        1# -> Fail#
        _ -> case indexCharOffAddr# buf 1# of
  ...
  where
    ...
    go !res = \case
      ...
      Path (r, n, d) ws t -> case length ws of
        l
          | res < l -> Path (r, n, Just $! d - res) ws (go (d - l) t)
          | otherwise -> Path (r, n, Nothing) ws (go (res - l) t)

compileTrie :: [(Int, String)] -> Trie' (Rule, Int, Maybe Int)
compileTrie = ensureBytes . fallbacks . pathify . mindepths . FlatParse.fromList
